...
def model(params, x):
    ...
    fidelities = [fidel_function(st) for st in states]
...
def main():
    ...
    plot_decision_boundary(lambda x: np.array([model(PARAMS, _x) for _x in x]), X, y)
    plt.savefig(
        "two-spirals-decision-boundary.png",
        dpi=150,
        format="png",
        transparent=True,
        bbox_inches="tight",
